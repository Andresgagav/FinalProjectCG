function Boss(atX, atY, velocity, movementRange, type, texture0, normal, lightSet, hero) {
    this.kMinionTex = "assets/minion_sprite2.png";
    this.kDelta = 0.1;
    this.kWidth = 8;
    this.kHeight = 10;
    this.kSpeed = 0.02;
    this.tickIntervalRemove = 0;
    this.kSpawnerTotal = 200;
    ...
    if (normal === null) {
        this.mBoss = new LightRenderable(texture0);
        ...
        this.mBoss = new IllumRenderable(texture0);
    ...
    var i;
    for (i = 3; i < lightSet.numLights(); i++) {
        this.mBoss.addLight(lightSet.getLightAt(i));
...
Boss.prototype.update = function () {
    ...
    this.mSpawnerTicks++;
    if (this.mSpawnerTicks > this.kSpawnerTotal && this.mAllMinions.length < 4) {
        this._spawnChaser();
        this.mSpawnerTicks = 0;
    }
...
Boss.prototype.buildSprite = function (atX, atY) {
    ...
    this.mBoss.getXform().setSize(this.kWidth, this.kHeight);
    this.mBoss.getXform().setZPos(2);
    this.mBoss.setElementPixelPositions(13, 113, 0, 128);
...
Boss.prototype._spawnChaser = function () {
    var x = this.getXform().getXPos() - 1;
    var y = this.getXform().getYPos() + 3;
    var m = new ChaserMinion(x, y, [0, 0], 0, 2, this.kMinionTex, null, this.mLightSet, 2, 2.5);
    this.mAllMinions.push(m);
};

Boss.prototype.remove = function () {
    this.mVisible = false;
    var index = this.mLightSet.mSet.indexOf(this.light);
    this.mLightSet.mSet[index].setLightTo(false);
    this.mLightSet.mSet.splice(index, 1);
};

Boss.prototype.getTickIntervalRemove = function (int) {
    if (this.tickIntervalRemove < int) {
        this.tickIntervalRemove++;
        return false;
    } else {
        return true;
    }
